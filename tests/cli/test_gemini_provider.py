...
def _make_cli(
    ...
    **kwargs: Any,
    ...
    monkeypatch.setattr("ductor_bot.cli.gemini_provider.GeminiCLI._find_cli_js", lambda _: "/path/to/gemini.js")
...
class TestInit:
    def test_trust_workspace_called(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestBuildCommand:
    ...
    def test_streaming_command(self, monkeypatch: pytest.MonkeyPatch) -> None:
        cli = _make_cli(monkeypatch)
        with patch("shutil.which", return_value="/usr/bin/gemini"):
            cmd = cli._build_command(streaming=True)
        assert "stream-json" in cmd

    def test_yolo_mode(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_allowed_tools(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestSend:
    ...
    async def test_error_exit(self, monkeypatch: pytest.MonkeyPatch) -> None:
        cli = _make_cli(monkeypatch)
...
class TestSendStreaming:
    async def test_happy_path(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        lines = [
            ...
            json.dumps({"type": "message", "role": "assistant", "content": "Live"}).encode() + b"\n",
            json.dumps({"type": "result", "status": "success", "response": "Live content"}).encode() + b"\n",
        ]
    ...
    async def test_abort_by_user(self, monkeypatch: pytest.MonkeyPatch) -> None:
        reg = ProcessRegistry()
